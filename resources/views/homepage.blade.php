    ...
    async function getData() {
        try {
            let response = await fetch(
                'https://api.openweathermap.org/data/2.5/weather?q=sentul,id&appid=3282175a32c9ea3ccd6e541b9510f24c'
                );
    ...
    getData().then(data => {
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            if (!weatherTextElement) {
                console.error(
                    "Elemen p.weathertext tidak ditemukan! Pastikan elemen ini ada dalam HTML.");
                return;
            }
        ...
        function updateTips() {
            const weatherTextElement = document.querySelector(
            "p.weathertext"); // Ambil elemen setiap kali updateTips dipanggil
            if (!weatherTextElement) {
                console.error("Elemen p.weathertext tidak ditemukan!");
                return;
            }
