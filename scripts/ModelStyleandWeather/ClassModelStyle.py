...
# class OneHotEncodingHandlerS:
#     def __init__(self, encoder_path=os.path.join(BASE_DIR, "models", "onehot_encoderS.pkl")):
...
# class UserOutfitModelS:
#     def __init__(self, user_id, default_data_path=os.path.join(BASE_DIR, "style_combinations.csv")):
#         self.user_id = user_id
#         self.global_ht_model_path = os.path.join(BASE_DIR, "models", "global_ht_modelS.pkl")
#         self.ht_model_path = os.path.join(BASE_DIR, "models", f"ht_model_S{user_id}.pkl")
#         self.data_path = os.path.join(BASE_DIR, "data", f"user_dataS{user_id}.csv")
#         self.encoder_handler = OneHotEncodingHandlerS()
...
#         if "adjusted_rate" not in data.columns:
#             return

#         categorical_cols = [col for col in data.columns if data[col].dtype == "object"]
#         self.encoder_handler.fit_encoder(data, categorical_cols)
        
#         y = data["adjusted_rate"]
#         X = self.encoder_handler.transform_data(data.drop(columns=["adjusted_rate"], errors="ignore"), categorical_cols)
...
#         new_data_encoded = self.encoder_handler.transform_data(new_data_df.drop(columns=["adjusted_rate"], errors="ignore"), categorical_cols)
...
#         self.ht_model.learn_one(new_data_encoded.iloc[0].to_dict(), new_data["adjusted_rate"])

#         if len(self.data) > 500:
#             X = self.encoder_handler.transform_data(self.data.drop(columns=["adjusted_rate"], errors="ignore"), categorical_cols)
#             y_true = self.data["adjusted_rate"]
